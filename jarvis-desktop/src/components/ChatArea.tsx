      ...
      {messages.length === 0 && !currentResponse ? (
        <div className="flex flex-col items-center justify-center h-full text-center">
          <div className="relative mb-6">
            <div className="w-24 h-24 rounded-full border-2 border-jarvis-primary/30 flex items-center justify-center animate-pulse-slow">
            ...
            <div className="absolute -inset-2 rounded-full border border-jarvis-primary/20 animate-spin-slow" />
          </div>
          <h2 className="text-2xl font-semibold text-white mb-2">J.A.R.V.I.S v2.0</h2>
          <p className="text-jarvis-textMuted max-w-md">
            Your personal AI assistant is ready. Ask me anything, write code, conduct research, or control your system.
          </p>
          <div className="mt-8 grid grid-cols-2 gap-3 text-sm">
            <div className="p-3 rounded-lg bg-jarvis-panel border border-jarvis-border/50 text-jarvis-textMuted">
        ...
        <>
            ...
            >
              <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
                msg.role === 'user' 
                  ...
                  : 'bg-jarvis-accent/10 border border-jarvis-accent/50'
              }`}>
                {msg.role === 'user' ? (
                  <User className="w-4 h-4 text-jarvis-primary" />
                ) : (
                  <Bot className="w-4 h-4 text-jarvis-accent" />
                )}
              </div>
              
              <div className={`flex-1 max-w-3xl ${msg.role === 'user' ? 'text-right' : ''}`}>
                {msg.role === 'user' ? (
                  <div className="inline-block px-4 py-3 rounded-2xl bg-jarvis-panel border border-jarvis-border text-left">
          ...
          {currentResponse && (
            <div className="flex gap-4">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-jarvis-accent/10 border border-jarvis-accent/50 flex items-center justify-center">
                <Bot className="w-4 h-4 text-jarvis-accent" />
              </div>
