#!/usr/bin/env python3
"""Jarvis Auto-Patcher — Autonomous self-healing script with human-in-the-loop approval.

Usage:
    python jarvis_auto_patcher.py
...
LLM_BASE_URL = os.environ.get("JARVIS_LLM_URL", "http://localhost:11434")
LLM_MODEL = os.environ.get("JARVIS_LLM_MODEL", "qwen3.5:27b")
LLM_API_KEY = os.environ.get("JARVIS_LLM_API_KEY", "")  # Set if using OpenAI-compatible
PROJECT_ROOT = Path(__file__).parent  # Jarvis project root
...
TOOL_READ_FILE = {
    ...
    "function": {
        "name": "read_file",
        "description": "Read the contents of a file from the Jarvis project. Returns the full text with line numbers.",
        "parameters": {
            "type": "object",
            "properties": {
                "filepath": {
                    "type": "string",
                    "description": "Relative path from project root, e.g., 'jarvis/voice_pipeline.py'"
                }
            },
            "required": ["filepath"],
        },
...
TOOL_LIST_FILES = {
    ...
    "function": {
        "name": "list_files",
        "description": "List files in a directory of the Jarvis project to discover file paths.",
        "parameters": {
            "type": "object",
            "properties": {
                "dirpath": {
                    "type": "string",
                    "description": "Relative directory path from project root, e.g., 'jarvis' or '.'"
                }
            },
            "required": [],
        },
...
def tool_read_file(filepath: str) -> str:
    ...
    if not full.exists():
        ...
        alt = PROJECT_ROOT / "jarvis" / filepath
        if alt.exists():
            full = alt
        else:
...
ANALYSIS_PROMPT = f"""You are an expert Python debugger. A bug has been reported in the Jarvis project.
