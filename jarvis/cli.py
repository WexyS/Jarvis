"""CLI arayüzü — Jarvis'ı başlatır (GUI varsayılan, --cli ile terminal)."""
...
from jarvis.config import JarvisConfig, load_config, ensure_directories
...
def _init_v2_sync(config: JarvisConfig):
...
def run_gui(config: JarvisConfig) -> None:
    ...
    try:
        from jarvis.gui_app import JarvisGUI
        app = JarvisGUI(pipeline=None, orchestrator=orchestrator, config=config)
        app.run()
    except Exception as e:
...
async def async_main(config: JarvisConfig) -> None:
    ...
    console.print("[green]✓[/] Jarvis v2.0 hazır. Yazmaya başlayın (/quit ile çıkış)\n")
    ...
    while True:
        try:
            ...
            with console.status("[dim]Jarvis düşünüyor...[/dim]", spinner="dots"):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Jarvis — Kişisel Yapay Zeka Asistanı")
