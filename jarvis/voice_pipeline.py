"""Jarvis Voice Pipeline — STT (Google + Whisper fallback) + VAD (Silero) + LLM (Ollama) + TTS (edge-tts) + Tool Calling + Global Skills/Agents."""
...
class VoicePipeline:
    """
    Jarvis sesli asistan pipeline'i.
    ...
    def _build_system_prompt(self) -> str:
        """Jarvis v2.0 sistem promptu — prompt.txt'den yüklenir."""
        # prompt.txt dosyasını bul
        prompt_file = None
        candidates = [
            Path(__file__).parent.parent / "jarvis" / "v2" / "core" / "prompt.txt",
        ...
        if not base:
            base = (
                "Sen Jarvis'sin. Kişisel yapay zeka asistanısın.\n"
    ...
    def _build_tool_handlers(self) -> dict:
        """Araç çalıştırıcıları — jarvis.actions modüllerini kullanır."""
        return {
    ...
    def _tool_weather_report(self, args: dict) -> str:
        ...
        try:
            from jarvis.actions.weather_report import run as weather_run
    ...
    def _tool_code_helper(self, args: dict) -> str:
        ...
        try:
            from jarvis.actions.code_helper import run as code_run
    ...
    def _tool_computer_settings(self, args: dict) -> str:
        ...
        try:
            from jarvis.actions.computer_settings import run as settings_run
