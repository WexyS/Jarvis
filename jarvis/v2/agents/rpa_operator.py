...
from jarvis.v2.core.types import AgentRole, AgentStatus, Task, TaskResult, TaskStatus, ToolCall
...
class RPAOperatorAgent(Agent):
    ...
    async def _autonomous_action(self, task: Task) -> TaskResult:
        ...
        # Check if we're still on Jarvis GUI (localhost) — switch away if so
        if "localhost" in screen_text.lower() or "127.0.0.1" in screen_text.lower() or "517" in screen_text:
            steps.append("⚠️ Detected Jarvis GUI — switching away with alt+tab")
            import pyautogui
            import time
            pyautogui.hotkey('alt', 'tab')
            time.sleep(1)
        ...
        messages = self._build_messages(
            ...
            f"   If you see 'localhost' or 'jarvis' on screen, do alt+tab FIRST.\n"
