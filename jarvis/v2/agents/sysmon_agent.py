...
from jarvis.v2.agents.base import Agent
from jarvis.v2.core.blackboard import Blackboard
from jarvis.v2.core.event_bus import EventBus
from jarvis.v2.core.llm_router import LLMRouter
from jarvis.v2.core.types import AgentRole, Task, TaskResult, TaskStatus
...
class SystemMonitorAgent(Agent):
    ...
    def __init__(
        ...
        try:
            self._cpu_threshold = float(
                os.getenv("JARVIS_SYSMON_CPU_THRESHOLD", self._cpu_threshold)
            )
            self._ram_threshold = float(
                os.getenv("JARVIS_SYSMON_RAM_THRESHOLD", self._ram_threshold)
            )
            self._disk_threshold = float(
                os.getenv("JARVIS_SYSMON_DISK_THRESHOLD", self._disk_threshold)
            )
        except ValueError:
    ...
    def _default_system_prompt(self) -> str:
        return (
            "You are Jarvis, a system monitoring assistant. You track CPU, "
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        text = (task.description + " " + task.intent).lower()
        ...
        try:
            ...
            logger.exception("SystemMonitorAgent task %s failed", task.id)
            return TaskResult(
                task_id=task.id,
                status=TaskStatus.FAILED,
                error=str(exc),
