"""Jarvis GUI — Mark-XXXV tarzı animasyonlu arayüz."""
...
class JarvisGUI:
    def __init__(self, pipeline=None, orchestrator=None, config=None):
        ...
        self.config = config
        self.root = tk.Tk()
        self.root.title("J.A.R.V.I.S — v2.0 Multi-Agent")
        ...
        if pipeline:
            self.write_log("SYS: JARVIS başlatılıyor (Qwen 2.5 14B + Claude + Multi-Agent)...")
    ...
    def _init_pipeline_bg(self):
        ...
        def worker():
            try:
                from jarvis.memory import ResponseCache, UserMemory
                from jarvis.voice_pipeline import VoicePipeline
                from pathlib import Path
                cfg = self.config
                cache = ResponseCache(
    ...
    def _connect_pipeline(self, pipeline):
        ...
        pipeline.on_assistant_text = lambda t: self.write_log(f"Jarvis: {t}")
    ...
    def _draw(self):
        ...
        elif self._jarvis_state == "TRANSCRIBING":
            sym = "⟳" if self.status_blink else "⟳"
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8), text="Jarvis v2.0  •  Multi-Agent  •  Qwen 2.5 14B + Claude")
