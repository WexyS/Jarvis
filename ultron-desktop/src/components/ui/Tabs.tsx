...
export function TabsTrigger({ value, children, title }: { value: string; children: React.ReactNode; title?: string }) {
  return (
    <TabsPrimitive.Trigger
      value={value}
      title={title}
      className="flex items-center justify-center p-2 rounded-md text-ultron-textMuted data-[state=active]:bg-ultron-card data-[state=active]:text-ultron-primary transition-colors"
    >
      {children}
    </TabsPrimitive.Trigger>
  );
