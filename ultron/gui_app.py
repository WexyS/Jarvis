"""Ultron GUI — Mark-XXXV tarzı animasyonlu arayüz."""
...
MODEL_BADGE = "QWEN 2.5 14B + CLAUDE"
SYSTEM_NAME = "Ultron"


class UltronGUI:
    def __init__(self, pipeline=None, orchestrator=None, config=None):
        self.pipeline = pipeline
        self.orchestrator = orchestrator
        self.config = config
        self.root = tk.Tk()
        self.root.title("Ultron — v2.0 Multi-Agent")
        ...
        LH = 110
        LOG_Y = H - LH - 80
        self.log_frame = tk.Frame(self.root, bg=C_PANEL, highlightbackground=C_MID, highlightthickness=1)
        self.log_frame.place(x=(W - LW) // 2, y=LOG_Y, width=LW, height=LH)

        # Sabit genişlikli font
        ...
        self.log_text = tk.Text(self.log_frame, fg=C_TEXT, bg=C_PANEL,
                                ...
                                wrap="word", font=fixed_font, padx=10, pady=6)
        ...
        # ── Pipeline varsa başlat ───────────────────────────────────────────
        if pipeline:
            self.write_log("SYS: ULTRON başlatılıyor (Qwen 2.5 14B + Claude + Multi-Agent)...")
            self._connect_pipeline(pipeline)
        elif self.config:
            # GUI kendi pipeline'ını oluştursun
            self.write_log("SYS: Pipeline yükleniyor... (arka plan)")
            self._init_pipeline_bg()
        else:
            self.write_log("SYS: Pipeline yok (sadece metin modu)")
    ...
    def _build_mute_button(self):
        ...
        BTN_X = 18
        BTN_Y = self.H - 70
        self._mute_canvas = tk.Canvas(self.root, width=BTN_W, height=BTN_H, bg=C_BG, highlightthickness=0, cursor="hand2")
    ...
    def _toggle_mute(self):
        ...
        # Pipeline'ın mikrofonunu da kapat/aç
        if self.pipeline:
            if self.muted:
                self.pipeline.disable_mic()
            else:
                self.pipeline.enable_mic()
        
        if self.muted:
            self.set_state("MUTED")
            self.write_log("SYS: Mikrofon susturuldu.")
        else:
            self.set_state("LISTENING")
            self.write_log("SYS: Mikrofon aktif.")
    ...
    def _build_input_bar(self, lw, y):
        ...
        fixed_font = tkfont.Font(family="Courier New", size=10)
        self._input_entry = tk.Entry(self.root, textvariable=self._input_var, fg=C_TEXT, bg="#000d12",
                                     insertbackground=C_TEXT, borderwidth=0, font=fixed_font,
        ...
        self._send_btn = tk.Button(self.root, text="GÖNDER ▸", command=self._on_input_submit,
    ...
    def _on_input_submit(self, event=None):
        ...
        if self.orchestrator:
            threading.Thread(target=self._process_v2, args=(text,), daemon=True).start()
        elif self.pipeline:
            # Pipeline'ın send_text'i arka planda çalışsın (GUI'yi bloklamaz)
            threading.Thread(target=self.pipeline.send_text, args=(text,), daemon=True).start()
        elif self.on_text_command:
            threading.Thread(target=self.on_text_command, args=(text,), daemon=True).start()

    # ── Pipeline bağlantısı ─────────────────────────────────────────────────

    def _init_pipeline_bg(self):
        """Arka planda pipeline oluştur ve bağla."""
        def worker():
            try:
                ...
                cfg = self.config
                cache = ResponseCache(
                    max_size=cfg.memory.cache_size,
                    ttl_seconds=cfg.memory.cache_ttl_hours * 3600,
                    similarity_threshold=cfg.memory.cache_similarity,
                )
                cache.set_persist_path(Path(cfg.memory.persist_dir) / "response_cache.json")
                pipe = VoicePipeline(
                    response_cache=cache,
                    user_memory=UserMemory(persist_dir=cfg.memory.persist_dir),
                    language=cfg.model.language,
                    auto_respond=True,
                )
                # Thread-safe GUI update
                def on_success():
                    self._connect_pipeline(pipe)
                    self._write_log_impl("SYS: Pipeline bağlandı ✓")
                def on_fail(err):
                    self._write_log_impl(f"SYS: Pipeline hatası: {err}")
                self.root.after(0, on_success)
            except Exception as e:
                import traceback
                tb = traceback.format_exc()
                def on_fail():
                    self._write_log_impl(f"SYS: Pipeline hatası: {e}")
                    print(f"Pipeline init error: {tb}")
                self.root.after(0, on_fail)

        threading.Thread(target=worker, daemon=True).start()

    def _connect_pipeline(self, pipeline):
        """Pipeline'ı GUI'ye bağla."""
        self.pipeline = pipeline
        pipeline.on_state_change = self.set_state
        pipeline.on_user_text = lambda t: self.write_log(f"You: {t}")
        pipeline.on_assistant_text = lambda t: self.write_log(f"Ultron: {t}")
        pipeline.on_error = lambda e: self.write_log(f"ERR: {e}")
        def runner():
            pipeline.enable_mic()
        threading.Thread(target=runner, daemon=True).start()

    def _process_v2(self, text: str) -> None:
        """Process text through v2 orchestrator with mode detection."""
        import asyncio

        # Anti-loop: same message sent twice in 3 seconds = ignore
        now = __import__('time').time()
        if hasattr(self, '_last_msg') and self._last_msg == text and (now - self._last_msg_time) < 3:
            return
        self._last_msg = text
        ...
        if text.startswith("/code "):
            ctx = {"intent": {"type": "code", "execute": True, "subtasks": [text[6:]]}}
            text = text[6:]
        elif text.startswith("/research "):
            ctx = {"intent": {"type": "research", "subtasks": [text[10:]]}}
            text = text[10:]
        elif text.startswith("/rpa "):
            ctx = {"intent": {"type": "rpa", "subtasks": [text[5:]]}}
            text = text[5:]

        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            response = loop.run_until_complete(self.orchestrator.process(text, context=ctx))
            loop.close()
            # Truncate response to prevent GUI freeze
            max_len = 3000
            if len(response) > max_len:
                response = response[:max_len] + "\n\n... (cevap kısaltıldı)"
            self.write_log(f"Ultron: {response}")
        except Exception as e:
    ...
    def set_state(self, state):
        # Thread-safe: main thread'e schedule et
        if threading.current_thread() is threading.main_thread():
            self._set_state_impl(state)
        else:
            self.root.after(0, self._set_state_impl, state)

    def _set_state_impl(self, state):
        self._ultron_state = state
        if state == "MUTED":
            self.status_text = "SUSTURULDU"
            self.speaking = False
        elif state == "SPEAKING":
            self.status_text = "KONUŞUYOR"
            self.speaking = True
        elif state == "THINKING":
            self.status_text = "DÜŞÜNÜYOR"
            self.speaking = False
        elif state == "LISTENING":
            self.status_text = "DİNLİYOR"
            self.speaking = False
        elif state == "TRANSCRIBING":
            self.status_text = "YAZIYA ÇEVRİLİYOR"
            self.speaking = False
        elif state == "CANCELLED":
            self.status_text = "İPTAL"
            self.speaking = False
        else:
    ...
    def _animate(self):
        ...
        self._draw()
        self.root.after(33, self._animate)  # 30fps (was 16=60fps) — CPU/GPU %50 azalır
    ...
    def _draw(self):
        ...
        for pr in self.pulse_r:
            ...
            r = int(pr)
            if self.muted:
                c.create_oval(FCX - r, FCY - r, FCX + r, FCY + r, outline=self._ac(255, 30, 80, pa // 3), width=2)
            else:
                c.create_oval(FCX - r, FCY - r, FCX + r, FCY + r, outline=self._ac(0, 212, 255, pa), width=2)
        ...
        c.create_arc(FCX - sr, FCY - sr, FCX + sr, FCY + sr, start=self.scan_angle, extent=arc_ext, outline=scan_col, width=3, style="arc")
        c.create_arc(FCX - sr, FCY - sr, FCX + sr, FCY + sr, start=self.scan2_angle, extent=arc_ext,
        ...
        # Köşe braketleri
        blen, bc = 22, self._ac(0, 212, 255, 200)
        ...
        for i in range(7, 0, -1):
            ...
            c.create_oval(FCX - r2, FCY - r2, FCX + r2, FCY + r2,
                          fill=self._ac(int(0 * frac), int(68 * frac), int(119 * frac), ga), outline="")
        ...
        c.create_text(W // 2, 44, text="Just A Rather Very Intelligent System", fill=C_MID, font=("Courier", 9))
        c.create_text(16, 31, text=MODEL_BADGE, fill=C_DIM, font=("Courier", 9), anchor="w")
        c.create_text(W - 16, 31, text=time.strftime("%H:%M:%S"), fill=C_PRI, font=("Courier", 14, "bold"), anchor="e")

        # Durum göstergesi
        sy = FCY + FW // 2 + 45
        if self.muted:
            stat, sc = "⊘ SUSTURULDU", C_MUTED
        elif self.speaking:
            stat, sc = "● KONUŞUYOR", C_ACC
        elif self._ultron_state == "THINKING":
            sym = "◈" if self.status_blink else "◇"
            stat, sc = f"{sym} DÜŞÜNÜYOR", C_ACC2
        elif self._ultron_state == "LISTENING":
            sym = "●" if self.status_blink else "○"
            stat, sc = f"{sym} DİNLİYOR", C_GREEN
        elif self._ultron_state == "TRANSCRIBING":
            sym = "⟳" if self.status_blink else "⟳"
            stat, sc = f"{sym} YAZIYA ÇEVRİLİYOR", C_ACC2
        else:
            sym = "●" if self.status_blink else "○"
        ...
        c.create_text(W - 16, H - 14, fill=C_DIM, font=("Courier", 8), text="[F4] SUSTUR", anchor="e")
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8), text="Ultron v2.0  •  Multi-Agent  •  Qwen 2.5 14B + Claude")

    # ── Log ─────────────────────────────────────────────────────────────────

    def write_log(self, text):
        # Tkinter thread-safe: main thread'e schedule et
        self.root.after(0, self._write_log_impl, text)

    def _write_log_impl(self, text):
        self.typing_queue.append(text)
        tl = text.lower()
        if tl.startswith("you:"):
            self.set_state("THINKING")
        elif tl.startswith("ultron:") or tl.startswith("ai:"):
    ...
    def _type_char(self, text, i, tag):
        if i < len(text):
            ...
            self.root.after(5, self._type_char, text, i + 1, tag)
        else:
